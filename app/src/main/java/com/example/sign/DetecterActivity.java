...
public class DetecterActivity extends Activity implements OnCameraListener, View.OnTouchListener, Camera.AutoFocusCallback, View.OnClickListener {
			...
			if (mImageNV21 != null) {
				...
				if (max > 0.6f) {
                  ...
                  /* runOnUiThread(new Runnable() {
                       @Override
                       public void run() {
                           Toast.makeText(DetecterActivity.this,"身份确认成功",Toast.LENGTH_SHORT).show();
                           finish();
                       }
                   });*/
                  sign();
	 ...
	 */
	private void sign(){
		SharedPreferences sharedPreferences = getSharedPreferences("rdata",MODE_PRIVATE) ;
		String cno = sharedPreferences.getString("cno",null) ;
		String week = sharedPreferences.getString("week",null) ;
		...
		RequestBody requestBody = new FormBody.Builder()
				...
				.add("num",num)
				.build() ;
		final Request request = new Request.Builder().url(Util.serviceUrl + "android/sign").post(requestBody).build();
